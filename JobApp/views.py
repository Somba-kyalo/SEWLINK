...
def propose_price(request, pk):
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request,
            'Agreed price submitted successfully.',
        )
        return redirect('JobApp:tailor_job_detail', pk=pk)
...
def create_order(request, pk):
    ...
    if not job.tailor:
        messages.error(
            request,
            'This job does not have an assigned tailor.',
        )
    ...
    if not job.agreed_price:
        messages.error(
            request,
            'The tailor must submit an agreed price first.',
        )
        return redirect('JobApp:job_detail', pk=pk)
    ...
    if hasattr(job, 'order'):
        messages.info(
            request,
            'An order already exists for this job.',
        )
        return redirect(
            'OrderApp:order_detail',
            pk=job.order.pk,
        )
